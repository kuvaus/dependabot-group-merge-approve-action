...
async function get_dependabot_pull_requests(options: Options) {
    try {
        ...
        const dependabot_pull_requests = pulls.data.filter(pr => pr.user?.login === botname);
...
async function auto_merge_combined_pull_request(options: Options, pr_number: number) {
  let attempts = 0;
